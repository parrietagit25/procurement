...
<head>
    ...
    <title>Test Final V2 - Sistema de Procurement</title>
    <style>
...
<body>
    <h1>Test Final V2 - Sistema de Procurement</h1>
    
    <div class="section">
        <h3>1. Test de Diagnóstico</h3>
        <button onclick="testEndpoint('/debug_router.php')">Debug Router</button>
        <button onclick="testEndpoint('/api_router.php')">Test API Router Directo</button>
    </div>
    
    <div class="section">
        <h3>2. Test de Endpoints de la API</h3>
    ...
    <div class="section">
        <h3>3. Test de Páginas del Sistema</h3>
        <button onclick="testPage('/')">Test Página Principal</button>
        <button onclick="testPage('/admin/dashboard.php')">Test Dashboard Admin</button>
        <button onclick="testPage('/supplier/dashboard.php')">Test Dashboard Supplier</button>
    </div>
    ...
    <script>
        ...
        async function testPage(page) {
            ...
            const resultDiv = document.createElement('div');
            resultDiv.className = 'info';
            resultDiv.innerHTML = `<strong>Probando página:</strong> ${page}`;
            resultsDiv.appendChild(resultDiv);
            ...
            try {
                ...
                resultDiv2.innerHTML = `
                    <strong>Resultado:</strong> ${response.status} ${response.statusText}<br>
                    <strong>Tipo de contenido:</strong> ${response.headers.get('content-type')}<br>
                    <strong>Tamaño:</strong> ${response.headers.get('content-length') || 'Unknown'} bytes
                `;
                resultsDiv.appendChild(resultDiv2);
